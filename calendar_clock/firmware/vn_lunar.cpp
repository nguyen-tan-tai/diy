...
uint32_t lunar_mm;
uint32_t lunar_yy;
uint32_t vn_lunar::floor(float v) {
  return (uint32_t) v;
}

uint32_t vn_lunar::getJulius(uint32_t _dd, uint32_t _mm, uint32_t _yy) {
  uint32_t a = floor((14 - _mm) / 12);
  uint32_t y = _yy + 4800 - a;
  uint32_t m = _mm + 12 * a - 3;
  uint32_t jd = _dd + floor((153 * m + 2) / 5) + 365 * y + floor(y / 4) - floor(y / 100) + floor(y / 400) - 32045;
  if (jd < 2299161) {
